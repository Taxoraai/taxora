...
/**
 * Fetch price history for a single token using /history endpoint (free tier compatible)
 */
async function fetchTokenHistory(
  ...
  // Fetch each date individually using /history endpoint (free tier works)
  for (const date of dates) {
    // Rate limiting
    const now = Date.now();
    ...
    const url = `${baseUrl}/coins/${tokenId}/history?date=${cgDate}&localization=false`;
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 429) {
          ...
          results[date] = retryData.market_data?.current_price?.usd ?? 0;
          continue;
        }
        results[date] = 0;
        continue;
      ...
      results[date] = data.market_data?.current_price?.usd ?? 0;
    } catch {
      results[date] = 0;
    }
  }
